...
var padStyle = function padStyle(_ref) {
  ...
  if (size && theme.button.size && theme.button.size[size] && theme.button.size[size].padding) {
    return css(["padding:", " ", ";"], theme.button.size[size].padding.vertical, theme.button.size[size].padding.horizontal);
  }
...
var plainStyle = function plainStyle() {
  return css(["outline:none;border:none;padding:0;text-align:inherit;"]);
};

var StyledButtonKind = styled.button.withConfig({
  displayName: "StyledButtonKind",
  componentId: "sc-10owomz-0"
